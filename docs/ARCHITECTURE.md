...
- `/api/activity` and `/api/activity/clear` use the existing Parent-access boundary, including a valid session whenever a PIN is configured; deleting a child profile also removes its retained activity, while tracking I/O failures never change application launch behavior
